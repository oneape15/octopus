...
public class ModelTagServiceImpl implements ModelTagService {
    ...
    public int insert(ModelTagDO model) {
        Assert.isTrue(model != null, "标签对象为空");
        Assert.isTrue(StringUtils.isNoneBlank(model.getName(), model.getRule()), "标签名或规则为空");
    ...
    public int edit(ModelTagDO model) {
        Assert.isTrue(model != null, "标签对象为空");
        Assert.isTrue(model.getId() != null && model.getId() > 0, "主键为空");
        Assert.isTrue(StringUtils.isNoneBlank(model.getName(), model.getRule()), "标签名或规则为空");
